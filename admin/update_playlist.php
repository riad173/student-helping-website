...
if(isset($_POST['submit'])){
   ...
   $update_playlist = $conn->prepare("UPDATE `playlist_all` SET title = ?, description = ?, status = ? WHERE id = ?");
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `playlist_all` SET thumb = ? WHERE id = ?");
...
<section class="playlist-form">
   ...
   <?php
         $select_playlist = $conn->prepare("SELECT * FROM `playlist_all` WHERE id = ?");
