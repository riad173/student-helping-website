...
$select_html = $conn->prepare("SELECT html FROM app_upload WHERE id = ? AND status = 'active' LIMIT 1");
        $select_html->execute([$get_id]);
        $html_file = $select_html->fetchColumn();


if ($html_file) : ?>
    <iframe src="../uploaded_files/app/<?= $html_file; ?>" width="100%" height="800px" frameborder="0"></iframe>
<?php else : ?>
    <p>No HTML file found for the specified ID.</p>
<?php endif; ?>

<?php
      $select_content = $conn->prepare("SELECT * FROM `app_upload` WHERE id = ? AND status = ?");
   ...
   <div class="video-details">
      <!-- Assuming you have a video element -->
      <!-- <video src="uploaded_files/app/<?= $fetch_content['html']; ?>" class="video" poster="uploaded_files/<?= $fetch_content['thumb']; ?>"></video> -->
   ...
   <?php
         ...
         echo '<p class="empty">No active content found!</p>';
      }
   ?>
