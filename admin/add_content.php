...
if(isset($_POST['submit'])){
   ...
   $thumb_folder = '../uploaded_files/video/'.$rename_thumb;
   ...
   $video_folder = '../uploaded_files/video/'.$rename_video;

   if($thumb_size > 2000000){
      $message[] = 'image size is too large!';
   }else{
      $add_playlist = $conn->prepare("INSERT INTO `video_upload`(id, tutor_id, playlist_id, title, description, video, thumb, status) VALUES(?,?,?,?,?,?,?,?)");
...
<section class="video-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <select name="playlist" class="box" required>
         ...
         <?php
         $select_playlists = $conn->prepare("SELECT * FROM `courses` WHERE tutor_id = ?");
