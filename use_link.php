...
<section class="visit-websites">
   <h1 class="heading">Websites to Visit</h1>
   <ul style="list-style: none;">
   <?php
      $select_websites = $conn->prepare("SELECT url FROM links_upload LIMIT 1");
      $select_websites->execute();
      while ($website = $select_websites->fetch(PDO::FETCH_ASSOC)) {
   ?>
      <li><a href="<?= $website['url']; ?>" target="_blank" class="inline-btn"><?= $website['url']; ?></a></li>
   <?php
      }
   ?>
   </ul>
...
<section class="watch-video">
   ...
   <?php
      $select_content = $conn->prepare("SELECT * FROM `links_upload` WHERE id = ? AND status = ?");
